...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    (Map[Char, Int]() /: chars)((m, c) =>
      m + (c -> (m.get(c) map { _ + 1 } getOrElse 1))
    ).iterator.toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match
    ...
    case x :: y :: ts =>
      makeCodeTree(x, y) :: ts sortBy { weight _ } // improve perf
  ...
  /** This function creates a code tree which is optimal to encode the text
    ...
    * them.
    */
  def createCodeTree(chars: List[Char]): CodeTree = until(singleton, combine)(
    ((makeOrderedLeafList _) compose times)(chars)
  ).head
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    // to be flatten instead of doing ::: which kills perf
  ...
  def convert(tree: CodeTree): CodeTable = tree match
    ...
    case Fork(l, r, cs, w) =>
      mergeCodeTables(left(convert(l)), right(convert(r)))

  val left: CodeTable => CodeTable = ct => prependBitPath(0)(ct)
  val right: CodeTable => CodeTable = ct => prependBitPath(1)(ct)
  val prependBitPath: Bit => CodeTable => CodeTable =
    b => ct => ct map { case (c, bs) => (c, b :: bs) }
