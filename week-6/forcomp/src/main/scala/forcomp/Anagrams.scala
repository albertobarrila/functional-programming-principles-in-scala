...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    val charList = w.toLowerCase().toCharArray().toList
    charList.groupBy(c => c).toList.map(t => (t._1, t._2.length)).sortBy(t=>t._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs => {
      val ls = for (
        xs_ls <- combinations(xs);
        x_cnt <- 1 to x._2
      ) yield (x._1, x_cnt) :: xs_ls
      ls.toList ++ combinations(xs)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match {
    case Nil => x
    case y1 :: ys => {
      val index = x.indexWhere(t => t._1 == y1._1)
      val cnt = x(index)._2
      subtract(x.updated(index, (y1._1, cnt - y1._2)).filter(t=>t._2!=0), ys)
    }
  }
